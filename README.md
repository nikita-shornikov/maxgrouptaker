...
```bash
# Клонируйте или перейдите в каталог проекта (тот, где лежит script2_web_add_to_group.py)
cd maxgrouptaker
...
Запускать скрипт нужно **из корня проекта** (из папки, где лежат `script2_web_add_to_group.py` и папка `src`). Тогда импорт модуля `maxgrouptaker` сработает без дополнительной установки.
